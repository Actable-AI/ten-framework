...
class BlazeSTTRealtimeConfig(BaseModel):
    """Configuration for Blaze Realtime STT Extension"""
...
class BlazeSTTRealtimeExtension:
    """
    Blaze Realtime STT Extension for the TEN Framework.

    Wraps the realtime STT WebSocket endpoint: /v1/stt/realtime

    Implements the TEN framework extension interface with async process()
    and get_metadata() methods, plus a streaming transcribe_stream() helper.
    """

    def __init__(
        self,
        config: Optional[Union[BlazeSTTRealtimeConfig, Dict[str, Any]]] = None,
    ):
        """
        Initialize Blaze Realtime STT Extension.

        Args:
            config: Configuration object (BlazeSTTRealtimeConfig) or dict from
                    the TEN framework. If None, uses environment variables.
        """
        if config is None:
            self.config = BlazeSTTRealtimeConfig()
        elif isinstance(config, dict):
            # Convert dict from TEN framework to BlazeSTTRealtimeConfig
            self.config = BlazeSTTRealtimeConfig(
